-- Challenge XP now counts ONLY quest claims within the window, not trophies.
--
-- Trophy XP is lifetime/cumulative and its timestamps can be disturbed by
-- account recovery / re-grants (which once dumped an entire trophy history into
-- one challenge's window). Quest claims are clean, in-window, same-period
-- actions — a fairer measure of effort during the 30 days, and immune to that
-- class of skew. Same return signature, so CREATE OR REPLACE is fine.
...
BEGIN
  ...
  xp AS (
    -- Quest-claim XP only (trophies deliberately excluded).
    SELECT prt.user_id,
      coalesce((SELECT sum(qc.xp_awarded) FROM public.quest_claims qc
                WHERE qc.user_id = prt.user_id
                  ...
                  AND qc.claimed_at < (v_end + 1)::timestamptz), 0) AS xp_window
    FROM parts prt
  ),
  wstart AS (
